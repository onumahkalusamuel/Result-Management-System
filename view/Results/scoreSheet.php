...
$total_page_count = ceil(($total_number_of_records/50));

?>
    <title><?=strtoupper($other->examinationTitle);?> | <?=strtoupper($other->classCategory);?> | <?=strtoupper($other->subjectTitle);?> - SCORE SHEET
    </title>
    <style>
        * {font-family: "Roboto";}
        ...
        <div id="printthisarea" style="min-width: 1400px">
            ...
            <?php $heading_2 = "<div class='heading2'>
                <p align='center'>
                    ...
                    <strong> EXAMINATION TITLE: <u>".strtoupper($other->examinationTitle)."</u> | CLASS: <u>".strtoupper($other->classCategory)."</u> | SUBJECT: <u>".strtoupper($other->subjectTitle)."</u></strong><br>
                </p>
            </div>
            <!-- Result section -->
                ...
                <table border=1 cellspacing='0' cellpadding='0' >
                    <tr>
                        ...
                        foreach($header->papers as $h):
                            $heading_2 .= "<th class='student-data table-heading'>".ucwords($h->paperTitle . " (" .$h->maximumScore .")" )."</th>";
                        endforeach;
                        
                        $heading_2 .= "<!-- Cumulative Assignment -->
                        <!-- Total -->
                        <th class='student-data table-heading'>".ucwords($header->total)."</th>
                        <!-- Percentage -->
                        <th class='student-data table-heading'>".ucwords($header->percentage)."</th>
                        <!-- Grade -->
                        <th class='student-data table-heading'>".ucwords($header->grade)."</th>
                        <!-- Position -->
                        <th class='student-data table-heading'>".ucwords($header->position)."</th>
                        <!-- Remark -->
                    ...
                    if($x === 1 || (($x-1)%50)===0) {
                        echo "<div class='pages' id='page-".(($x-1)/50)."'>";
                        echo $heading_1;
                        echo $heading_2;
                    };
                    ?>
                    <tr>
                        ...
                        <!-- Assignments -->
                        <?php foreach($b->papers as $h) :?>
                        <td class="student-data"><?=$h;?></td>
                        <?php endforeach;?>
                        <td class="student-data"><?=$b->total;?></td>
                        <td class="student-data"><?=$b->percentage;?></td>
                        <!-- Grade -->
                        <td class="student-data"><?=strtoupper($b->grade);?></td>
                        <!-- Position -->
                        <td class="student-data"><?=ucwords($b->position);?></td>
                        <!-- Remark -->
                        <td class="student-data table-heading">&nbsp;&nbsp;&nbsp;&nbsp;</td>
                    </tr>
                    <?php
                    if(($x%50)===0 && $x !== $total_number_of_records) {
                        echo "</table>";
                        echo "<br>";
                        echo "<div class='text-center paging'><strong>Page ".($x/50)." of ".$total_page_count." </strong></div>";
                        echo "</div>";
                        echo "</div>";
                        echo "<br><br><br>";
                    };
